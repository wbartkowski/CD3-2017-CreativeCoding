...
void draw() {
  ...
  {
    float xp = punkty[nr].x-skala*noise(nr*0.1+t);;
    float yp = punkty[nr].y-skala*noise(100+nr*0.1+t);;
    float xk = punkty[nr].x+skala*noise(1000+nr*0.1+t);
    float yk = punkty[nr].y+skala*noise(10000+nr*0.1+t);
    line(xp, yp, xk, yk);
    nr = nr + 1;
